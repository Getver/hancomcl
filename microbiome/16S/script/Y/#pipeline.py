...
def pipeline_vagina(sample, input_abundance, basicT, abundanceT) :
    ## 데이터 파싱 : genus/species 로 풍부도 계산하기
    genus_ab, species_ab = PD.prepare_data(sample, input_abundance)
    ## target 가져오기
    filterG, filterS = PD.target_select(genus_ab, species_ab, abundanceT)
    ...
    disease_score_dict = CS.disease_score(scoreT, CST_type, abundanceT)
    ## @@ 항생제 풍부도 반환했음
