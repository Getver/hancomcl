...
## 질환과 질환에 관여하는 미생물 표시(Y)
# diseaseT = pd.read_csv('info/diseaseTable.txt', sep='\t')
...
# sampleL = pd.read_csv('/disk0/sm/microbiome/16S/'+path+'/meta-data.tsv', sep='\t')['sample-id'].to_list()
...
for sample in sampleL:
    results[sample] = P.pipeline_vagina(sample, input_abundance, basicT, abundanceT)
